...
export default function Thinktank() {
  ...
  async function handleNewIdea() {
    ...
    try {
      ...
      setCaptureText('')
      // HMR will refresh data automatically
    } catch { /* silently fail */ }
    ...
    <AppShell title="Thinktank" ledColor="p" kaniContext="thinktank">
      ...
      <div className="krow">
        ...
        <div className="capc cf">
          ...
          <input className="in" placeholder="Neue Idee erfassen..." value={captureText} onChange={e => setCaptureText(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && captureText.trim()) handleNewIdea() }} />
          <button onClick={handleNewIdea} disabled={capturing}>{capturing ? '...' : '+ Idee'}</button>
        </div>
      </div>
            ...
            <div className="det cf">
              ...
              <div className="det-body">
                ...
                <div style={{ display: 'flex', gap: 6 }}>
                  <button className="det-btn" style={{ background: 'var(--g)', color: '#fff', boxShadow: '0 3px 8px var(--gg)' }} onClick={async () => {
                    if (!selected.id) return
                    await fetch(`/api/idea/${selected.id}/to-project`, { method: 'POST' })
                    navigate(`/projekte/${selected.id}`)
                  }}>
                    {'\u2192'} Projekt starten
                  </button>
                  <button className="det-btn" style={{ background: 'var(--bl)', color: '#fff', boxShadow: '0 3px 8px var(--blg)' }} onClick={async () => {
                    if (!selected.id) return
                    await fetch(`/api/idea/${selected.id}/research`, { method: 'POST' })
                  }}>
                    {'\u{1F52C}'} Research
                  </button>
                  <button className="det-btn" style={{ background: 'var(--bg)', color: 'var(--tx3)', boxShadow: '2px 2px 6px var(--shdr), -2px -2px 6px var(--shl)' }} onClick={async () => {
                    if (!selected.id) return
                    await fetch(`/api/idea/${selected.id}/park`, { method: 'POST' })
                  }}>
                    Archivieren
                  </button>
                </div>
